...
export function BottomNavigation() {
  const mobileNav = (
    ...
    >
      <GlassChromeBar as="nav" className="px-1 py-1" aria-label="Primary">
          ...
          {items.map(({ to, label, icon: Icon }) => (
            <li key={to}>
              <NavLink
                ...
                className={({ isActive }) =>
                  cn(
                    'motion-btn flex min-h-12 flex-col items-center justify-center gap-0.5 rounded-[8px] text-[9px] font-semibold uppercase tracking-[0.08em]',
                    isActive
              ...
              >
                {({ isActive }) => (
                  <>
                    <Icon className="h-4 w-4" strokeWidth={isActive ? 2.25 : 1.6} />
                    {label}
                  </>
                )}
              </NavLink>
          ...
          {items.slice(0, 4).map(({ to, label, icon: Icon }) => (
            <li key={to}>
              <NavLink
                ...
                className={({ isActive }) =>
                  cn(
                    'motion-btn flex items-center gap-3 rounded-[8px] px-3 py-2.5 text-[12px] font-medium uppercase tracking-[0.08em]',
                    isActive
                      ? 'motion-nav-active bg-white/[0.1] text-white'
